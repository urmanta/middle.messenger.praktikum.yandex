...
import {
  validateEmail, validateLogin, validatePhone, validateName, withRouter,
} from '../../utils';
...
class Profile extends Block<ProfileProps, ProfileChildren> {
  ...
  init() {
    ...
    const MailInput = new ProfileField({
      title: 'Почта', name: 'mail', value: this.props.email, onBlur: (e: Event) => onChangeBind(e, validateEmail), readonly: !this.props.isEditMode,
    });
    const LoginInput = new ProfileField({
      title: 'Логин', name: 'login', value: this.props.login, onBlur: (e: Event) => onChangeBind(e, validateLogin), readonly: !this.props.isEditMode,
    });
    const NameInput = new ProfileField({
      title: 'Имя', name: 'first_name', value: this.props.first_name, onBlur: (e: Event) => onChangeBind(e, validateName), readonly: !this.props.isEditMode,
    });
    const SecondNameInput = new ProfileField({
      title: 'Фамилия', name: 'second_name', value: this.props.second_name, onBlur: (e: Event) => onChangeBind(e, validateName), readonly: !this.props.isEditMode,
    });
    const DisplayNameInput = new ProfileField({
      title: 'Имя в чате', name: 'display_name', value: this.props.display_name, onBlur: (e: Event) => onChangeBind(e, validateName), readonly: !this.props.isEditMode,
    });
    const PhoneInput = new ProfileField({
      title: 'Телефон', name: 'phone', value: this.props.phone, onBlur: (e: Event) => onChangeBind(e, validatePhone), readonly: !this.props.isEditMode,
    });
    const SaveButton = new Button({
      label: 'Сохранить', page: 'profile', className: 'profile-page__save-button', onClick: onSaveBind, disabled: !this.props.isFormValid,
    });
