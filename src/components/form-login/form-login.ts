...
class FormLogin extends Block<FormLoginProps, FormLoginChildren> {
  ...
  init() {
    ...
    const InputLogin = new Input({
      label: 'Логин', name: 'login', value: undefined, onBlur: (e: Event) => onChangeBind(e, validateLogin), className: 'login-page__input',
    });
    const FormPassword = new Input({
      label: 'Пароль', name: 'password', value: undefined, type: 'password', onBlur: (e: Event) => onChangeBind(e, validatePassword), className: 'login-page__input',
    });
    const ButtonLogin = new Button({
      label: 'Авторизироваться', type: 'primary', page: 'chat', onClick: onLoginBind, disabled: !this.props.isFormValid,
    });
