...
import {
  validateEmail, validateLogin, validatePassword, validatePhone, validateName,
} from '../../utils';
...
export default class FormRegistration extends Block<FormRegistrationProps, FormRegistrationChildren> {
  ...
  init() {
    ...
    const InputEmail = new Input({
      label: 'Почта', name: 'email', onBlur: (e: Event) => onChangeBind(e, validateEmail), className: 'login-page__input',
    });
    const InputLogin = new Input({
      label: 'Логин', name: 'login', onBlur: (e: Event) => onChangeBind(e, validateLogin), className: 'login-page__input',
    });
    const InputName = new Input({
      label: 'Имя', name: 'first_name', onBlur: (e: Event) => onChangeBind(e, validateName), className: 'login-page__input',
    });
    const InputLastName = new Input({
      label: 'Фамилия', name: 'second_name', onBlur: (e: Event) => onChangeBind(e, validateName), className: 'login-page__input',
    });
    const InputPhone = new Input({
      label: 'Телефон', name: 'phone', onBlur: (e: Event) => onChangeBind(e, validatePhone), className: 'login-page__input',
    });
    const InputPassword = new Input({
      label: 'Пароль', name: 'password', onBlur: (e: Event) => onChangeBind(e, validatePassword), className: 'login-page__input', type: 'password',
    });
    const InputPasswordConfirm = new Input({
      label: 'Пароль еще раз', name: 'password_confirm', onBlur: (e: Event) => onChangeBind(e, validatePassword), className: 'login-page__input', type: 'password',
    });
    const ButtonRegistrate = new Button({
      label: 'Зарегистрироваться', type: 'primary', page: 'chat', onClick: onSigninBind, disabled: !this.props.isFormValid,
    });
